...
type GateNodeType = Node<DAGNodeData, 'gate'>;

export function GateNode({ data }: NodeProps<GateNodeType>) {
  return (
    <div className="dag-node dag-node--gate">
      ...
      <div className="dag-node__body">
        <span className="dag-node__hint">{(data.config.gateType as string) ?? 'Quality gate'}</span>
      </div>
