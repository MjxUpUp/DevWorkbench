import { Handle, Position } from '@xyflow/react';
import type { NodeProps, Node } from '@xyflow/react';
import type { DAGNodeData } from '../../../stores/orchestrateStore';

type MergeNodeType = Node<DAGNodeData, 'merge'>;

export function MergeNode({ data }: NodeProps<MergeNodeType>) {
  return (
    <div className="dag-node dag-node--merge">
      <Handle type="target" position={Position.Left} />
      <div className="dag-node__header">
        <span className="dag-node__icon">⫷</span>
        <span className="dag-node__title">{data.label}</span>
      </div>
      <div className="dag-node__body">
        <span className="dag-node__hint">Merge branches</span>
      </div>
      <Handle type="source" position={Position.Right} />
    </div>
  );
}
