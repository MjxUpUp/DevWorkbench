...
export function ConfigCenter() {
  const open = useNavigationStore((s) => s.configCenterOpen);
  const setOpen = useNavigationStore((s) => s.setConfigCenterOpen);
  ...
  // Load config when opening with a project selected
  useEffect(() => {
    if (open && activeProject) {
      loadConfig(activeProject.path);
    }
  }, [open, activeProject, loadConfig]);
