...
use crate::trace::db::{
    get_trace_settings, list_traces_for_session, maybe_vacuum, prune_old_traces,
    set_trace_retention, LlmTraceRow, TraceSettings,
};
...
#[tauri::command]
pub async fn get_trace_settings_cmd(
    db: State<'_, DbState>,
) -> Result<TraceSettings, AppError> {
    let conn = db.get().map_err(|e| AppError::Internal(format!("db lock: {e}")))?;
